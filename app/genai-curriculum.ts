...
import { buildRoundFiveGenAILab } from "./round5-genai-labs";
import { buildRoundSixGenAILab } from "./round6-genai-labs";
import { buildRoundSevenGenAILab } from "./round7-genai-labs";
...
import { buildRoundTwelveGenAILab } from "./round12-genai-labs";
import { buildRoundThirteenGenAILab } from "./round13-genai-labs";
import { buildRoundFourteenGenAILab } from "./round14-genai-labs";
import { buildRoundFifteenGenAILab } from "./round15-genai-labs";
...
export function buildGenAILab(topic: GenAITopic, required = false, worldName = "GenAI world"): GenAILabSpec {
  ...
  const roundFiveLab = buildRoundFiveGenAILab(topic, required, worldName);
  if (roundFiveLab) return roundFiveLab;
  const roundSixLab = buildRoundSixGenAILab(topic, required, worldName);
  if (roundSixLab) return roundSixLab;
  const roundSevenLab = buildRoundSevenGenAILab(topic, required, worldName);
  if (roundSevenLab) return roundSevenLab;
  const roundEightLab = buildRoundEightGenAILab(topic, required, worldName);
  if (roundEightLab) return roundEightLab;
  const roundNineLab = buildRoundNineGenAILab(topic, required, worldName);
  ...
  const roundTwelveLab = buildRoundTwelveGenAILab(topic, required, worldName);
  if (roundTwelveLab) return roundTwelveLab;
  const roundThirteenLab = buildRoundThirteenGenAILab(topic, required, worldName);
  if (roundThirteenLab) return roundThirteenLab;
  const roundFourteenLab = buildRoundFourteenGenAILab(topic, required, worldName);
  if (roundFourteenLab) return roundFourteenLab;
  const roundFifteenLab = buildRoundFifteenGenAILab(topic, required, worldName);
  if (roundFifteenLab) return roundFifteenLab;
