...
function transformArtifact(lesson: CloudLesson): CloudArtifact {
  ...
  return { kind: "kubernetes", label: "Python transform", filename: "transform.py", language: "Python", brief: `${enrichment.scenario} Make this decision explicit: ${enrichment.decision} Repair deterministic record handling. Code remains inert text and is never executed.`, starter, solution, checks: [c("Exact numeric conversion", "Use Decimal instead of binary float for the amount.", s => /Decimal\(str\(row\["amount"\]\)\)/.test(s)), c("Identity is validated", "Reject missing record_id values.", s => /missing record_id/.test(s)), c("Rejected rows retain reasons", "Return rejected rows with a reason and run id.", s => /"rejected"/.test(s) && /"reason"/.test(s) && /source_run_id/.test(s)), c("Reconciliation count is returned", "Return input_count with accepted and rejected collections.", s => /"input_count"\s*:\s*len\(rows\)/.test(s) && /"accepted"/.test(s))] };
}

export function getDataArtifact(paceId: DataPaceId, lesson: CloudLesson): CloudArtifact {
  const title = lesson.title;
  if (/Directed Acyclic|DAG|Scheduling|Retries|Backfills|Pipeline|Workflow|Logging|Recovery|SLOs|Capacity|Self-service|Platform/.test(title)) return workflowArtifact(lesson);
  if (/Contract|Schema Evolution|Classification|PII|Encryption|Access|Catalog|Ownership|Lineage|Quality Framework|Governed|Event-driven|Topics, Partitions|Producers, Consumers|Event Time/.test(title)) return contractArtifact(lesson);
  if (/Structured|CSV|JSON|Parquet|Types|Nulls|Encoding|Extracting|Cleaning|Distributed|Transformations|Shuffles|Join|Watermarks|Change Data Capture|Event Stream/.test(title)) return transformArtifact(lesson);
  void paceId;
  return sqlArtifact(lesson);
}
