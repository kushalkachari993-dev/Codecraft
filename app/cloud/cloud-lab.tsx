...
import { evaluateSecurityArtifact, getSecurityArtifact } from "../security/artifacts";
import { getSecurityCheckpoints } from "../security/checkpoints";
import { getSecurityEvidenceExercise } from "../security/evidence";
import { securityLessonXp } from "../security/progress";
import { SECURITY_PATH_TOTAL, getSecurityPath, isSecurityWorldProject } from "../security/track";
...
const lessonPath = (trackId: "cloud" | "backend" | "frontend" | "data" | "security", paceId: CloudPaceId, id: number) => "/lesson/" + trackId + "/" + paceId + "/" + id;
...
export default function CloudLab({ paceId, lesson, completed, daily = false, trackKind = "cloud", onComplete }: {
  paceId: CloudPaceId; lesson: CloudLesson; completed: boolean; daily?: boolean; trackKind?: "cloud" | "backend" | "frontend" | "data" | "security"; onComplete: () => void;
}) {
  const backend = trackKind === "backend";
  const frontend = trackKind === "frontend";
  const data = trackKind === "data";
  const security = trackKind === "security";
  useLearningVisit(trackKind, paceId, lesson.id, lesson.title, !daily);
  const trackLabel = security ? "Cybersecurity Engineering" : data ? "Data Engineering" : frontend ? "Frontend Web Development" : backend ? "Backend Engineering" : "Cloud Engineering";
  const trackShortLabel = security ? "Security" : data ? "Data" : frontend ? "Frontend" : backend ? "Backend" : "Cloud";
  const lessonTotal = security ? SECURITY_PATH_TOTAL : data ? DATA_PATH_TOTAL : frontend ? FRONTEND_PATH_TOTAL : backend ? BACKEND_PATH_TOTAL : CLOUD_PATH_TOTAL;
  const path = security ? getSecurityPath(paceId) : data ? getDataPath(paceId) : frontend ? getFrontendPath(paceId) : backend ? getBackendPath(paceId) : getCloudPath(paceId);
  const checkpoints = security ? getSecurityCheckpoints(paceId, lesson) : data ? getDataCheckpoints(paceId, lesson) : frontend ? getFrontendCheckpoints(paceId, lesson) : backend ? getBackendCheckpoints(paceId, lesson) : getCloudCheckpoints(paceId, lesson);
  const evidence = security ? getSecurityEvidenceExercise(paceId, lesson) : data ? getDataEvidenceExercise(paceId, lesson) : frontend ? getFrontendEvidenceExercise(paceId, lesson) : backend ? getBackendEvidenceExercise(paceId, lesson) : getCloudEvidenceExercise(paceId, lesson);
  const artifact = security ? getSecurityArtifact(paceId, lesson) : data ? getDataArtifact(paceId, lesson) : frontend ? getFrontendArtifact(paceId, lesson) : backend ? getBackendArtifact(paceId, lesson) : getCloudArtifact(paceId, lesson);
  const evaluateArtifact = security ? evaluateSecurityArtifact : data ? evaluateDataArtifact : frontend ? evaluateFrontendArtifact : backend ? evaluateBackendArtifact : evaluateCloudArtifact;
  const worldProject = security ? isSecurityWorldProject(paceId, lesson.id) : data ? isDataWorldProject(paceId, lesson.id) : frontend ? isFrontendWorldProject(paceId, lesson.id) : backend ? isBackendWorldProject(paceId, lesson.id) : isCloudWorldProject(paceId, lesson.id);
  const decisionLabel = security ? "security" : data ? "data" : frontend ? "interface" : "architecture";
  const contractLabel = security ? "security boundary" : data ? "data contract" : frontend ? "rendering contract" : "Relay plan";
  const simulationLabel = security ? "SECURITY SIMULATOR" : data ? "PIPELINE SIMULATOR" : frontend ? "EXPERIENCE SIMULATOR" : "ARCHITECTURE SIMULATOR";
  ...
  const complete = () => {
    ...
    if (!checkpointPassed) {
      setSaveError(`Pass all three ${decisionLabel} decisions before completing this lesson.`);
      return;
    }
    if (!evidencePassed) {
  ...
  const rewardXp = daily ? DAILY_QUEST_XP : security ? securityLessonXp(lesson.id, paceId) : data ? dataLessonXp(lesson.id, paceId) : frontend ? frontendLessonXp(lesson.id, paceId) : backend ? backendLessonXp(lesson.id, paceId) : cloudLessonXp(lesson.id, paceId);
      ...
      <div className="lesson-workspace cloud-workspace">
        <article className="lesson-content cloud-reading" id="cloud-reading" aria-labelledby="cloud-objective">
          ...
          <section className="cloud-checkpoint" id="cloud-checkpoint" aria-labelledby="cloud-checkpoint-title">
            <div className="theory-heading"><span>STEP 3 · THREE REQUIRED DECISIONS</span><h2 id="cloud-checkpoint-title">Reason through the {contractLabel}</h2></div>
        ...
        <section className="coding-station cloud-lab" aria-labelledby="cloud-lab-title">
          ...
          {worldProject && lesson.projectStages && <section className="frontend-project-brief" aria-labelledby="frontend-project-brief-title"><div><small>CONNECTED PROJECT BRIEF</small><h3 id="frontend-project-brief-title">{security ? "Map, defend, test, and recover the system" : "Build, test, and defend the whole experience"}</h3><p>Each stage reuses decisions from this world. Treat the evidence as one continuous review—not four unrelated exercises.</p></div><ol>{lesson.projectStages.map((stage, index) => <li key={stage.title}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{stage.title}</strong><p>{stage.brief}</p><small>EVIDENCE · {stage.evidence}</small></div></li>)}</ol></section>}
          <p className="cloud-simulation-note"><strong>Local, static training only.</strong> {security ? "All targets, accounts, evidence, and incidents are fictional. No scan, attack, submitted artifact, or external service is run." : data ? "No SQL, workflow, transform, or contract is executed; no data platform or credentials are used." : frontend ? "No submitted markup, style, script, or policy is executed or published." : backend ? "No server, database, queue, external API, or credentials are used." : "No provider login, live infrastructure, external API, credentials, or cloud charges."} Every scanner runs deterministically in this page.</p>
          {worldProject && <section className="cloud-capstone-stages" aria-labelledby="cloud-capstone-title">
            <div><small>WORLD PROJECT</small><h3 id="cloud-capstone-title">Four-stage {security ? "security" : frontend ? "product experience" : "architecture"} and recovery review</h3><p>Complete each gate in order, from design reasoning through incident evidence and two independent repairs.</p></div>
            <ol>
              <li className={checkpointPassed ? "done" : "active"}><span>{checkpointPassed ? "✓" : "1"}</span><div><strong>{security ? "Security" : frontend ? "Interface" : "Architecture"} decisions</strong><small>Three scenarios</small></div></li>
              ...
              <li className={result?.passed ? "done" : checkpointPassed && evidencePassed && artifactPassed ? "active" : ""}><span>{result?.passed ? "✓" : "4"}</span><div><strong>{security ? "Boundary" : frontend ? "Experience" : "System"} verification</strong><small>{contractLabel}</small></div></li>
            </ol>
          </section>}
          ...
          <div className="cloud-stage-heading"><span>{simulationLabel}</span><h3>Repair the {contractLabel}</h3><p>Choose JSON, HCL, or YAML and repair the same fictional {security ? "security" : data ? "pipeline" : frontend ? "interface" : "system"} model.</p></div>
          ...
          <div className="cloud-editor-heading"><label htmlFor="cloud-plan">{contractLabel} · {formatLabel(format)}</label><span>Safe flat training syntax</span></div>
          ...
          {completed ? (
            ...
            <div className="cloud-complete-action">
              ...
              <p>{canComplete ? "All decision, evidence, artifact, and system checks passed. Save your completion." : `Finish all four gates: ${decisionLabel} decisions, evidence triage, static artifact review, and the ${contractLabel} simulation.`}</p>
            </div>
          )}
